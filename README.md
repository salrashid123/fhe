...
3. Rider encrypts the xy coordinates using `pk`:
   - `pk(x1, y1)` -> `(x1', y1')`
5. Rider sends encrypted `(x1', y1')`  and `pk` to server
6. Server sends `pk` to 50 drivers each in random positions in the grid.
7. Each driver encrypts their respective location using `pk`:
   - `pk(x2, y2)` -> `(x2', y2')`
9. Each driver each sends `(x2', y2')` to the server
10. Server calculates over the still encrypted data the distance from the rider to each driver

    - `d' = (x2'-x1')^2  + (y2'-y1')^2`  

9. Server sends the encrypted result for each driver's distance back to the rider
10. Rider decrypts distance from each driver using the `sk`:
    - `sk(d')` -->  `d`
12. Rider picks the closest driver
